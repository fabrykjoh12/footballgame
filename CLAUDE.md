...
> `src/data/managers.ts` + `src/lib/playerDb.ts` — **543 curated players** (stable
...
| **Central player database** (543 players, stable facts incl. birthYear) + canonical club registry (228 clubs, alias resolution) + pure query layer (playedForBoth, byNationality/Position/League, careerChain, cluesFor) | `src/data/players.ts`, `src/data/clubs.ts`, `src/lib/playerDb.ts` |
...
| **Rondo** (NEW — head-to-head naming duel: a shared rule is drawn from the Scout catalog and the two sides take turns naming players who fit it; a wrong name, a repeat, or a timeout is a turnover → the opponent scores a goal, first to 3 wins. Reuses the Scout category catalog + name resolution/autocomplete; own pure engine (rally/match reducers, turn switching, `concludeRally`/`nextRondoRally`) + info-greedy CPU (difficulty = miss-chance) + seeded daily **endurance** variant with a played-day streak; vs CPU · pass-and-play · daily; views `#rondo`/`#daily-rondo`, storage `bk_rondo_v1`. Turn-based + secretless, so an online layer can follow the Mystery/Scout pattern later) | `src/lib/rondo/` (`engine`, `cpu`, `daily` — all tested incl. a full-match integration test), `src/components/rondo/RondoGame.tsx` |
