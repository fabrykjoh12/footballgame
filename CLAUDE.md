...
> `src/data/managers.ts` + `src/lib/playerDb.ts` — **663 curated players** (stable
...
| **Central player database** (663 players, stable facts incl. birthYear) + canonical club registry (228 clubs, alias resolution) + pure query layer (playedForBoth, byNationality/Position/League, careerChain, cluesFor) | `src/data/players.ts`, `src/data/clubs.ts`, `src/lib/playerDb.ts` |
...
| **Rondo** (NEW — head-to-head naming duel: a shared rule is drawn from the Scout catalog and the two sides take turns naming players who fit it; a wrong name, a repeat, or a timeout is a turnover → the opponent scores a goal, first to 3 wins. Reuses the Scout category catalog + name resolution/autocomplete; own pure engine (rally/match reducers, turn switching, `concludeRally`/`nextRondoRally`) + info-greedy CPU (difficulty = miss-chance) + seeded daily **endurance** variant with a played-day streak; vs CPU · pass-and-play · daily · **online 1v1**; views `#rondo`/`#daily-rondo`, storage `bk_rondo_v1`. **Online 1v1 BUILT** (`ablyRondoService.ts` + `RondoOnlineGame.tsx`, host-authoritative over `bk-rondo-<CODE>`; no secrets so the full state syncs; the turn clock is host-authoritative — host stamps a `turnDeadline` per turn + a fallback timer, and a `timeout` is only honoured once the deadline passed; anti-spoof clientId check) — **needs two-device testing** like Scout/Mystery) | `src/lib/rondo/` (`engine`, `cpu`, `daily`, `online` — all tested incl. full-match integration + a two-player online-reducer match sim), `src/components/rondo/RondoGame.tsx`, `src/components/rondo/RondoOnlineGame.tsx`, `src/services/ablyRondoService.ts` |
...
| **Cloud Health self-test** (pure orchestrator; round-trips every Firestore social path against the signed-in account — sync/profile/username-search/friend-code/invites/leaderboard/leagues — and maps each failure code to the exact console fix; backend injected so it's fake-tested; non-destructive via per-uid `selftest-{uid}` namespaces filtered from real UI; renders in Settings only when signed in) | `src/lib/cloudHealth.ts`(+test), `src/lib/usernamePrefix.ts`(+test), `src/components/settings/CloudHealthPanel.tsx`, `firestore.rules` |
