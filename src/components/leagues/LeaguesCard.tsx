                    ...
                    {leagues.map((l) => (
                      <li key={l.id}>
                        <button
                          ...
                          className="answer-press flex w-full items-center gap-2 rounded-xl border border-white/10 bg-white/[0.03] px-3 py-2 text-left hover:bg-white/[0.03]"
                        >
...
function StandingsView({ leagueId, onBack }: { leagueId: string; onBack: () => void }) {
  ...
  return (
    <div>
      ...
      {data === 'loading' ? (
        <p className="py-8 text-center text-sm text-white/55">Loading table…</p>
      ) : !data ? (
        <p className="py-8 text-center text-sm text-white/55">Couldn’t load this league.</p>
      ) : (
        <>
          <h2 className="text-center font-display text-xl font-bold">{data.league.name}</h2>
          <div className="mt-1 mb-4 flex items-center justify-center gap-2 text-xs text-white/55">
            ...
            <button
              ...
              className="rounded-full p-1 text-white/55 hover:bg-white/[0.05] hover:text-white"
            >
          ...
          {data.standings.every((r) => r.played === 0) ? (
            ...
            <ol className="flex max-h-[55vh] flex-col gap-1.5 overflow-y-auto">
              {data.standings.map((row) => (
                ...
                >
                  <span className="w-6 text-center font-mono text-sm font-bold text-white/65">
                    {row.rank}
                  </span>
                  <span className="min-w-0 flex-1">
