  ...
  return (
    <div className="flex flex-col gap-4 py-4 animate-fade-in">
      ...
      <div className="text-center">
        <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/[0.04] px-3 py-1 text-xs text-white/65">
          🕵️ Deduction duel
        </div>
      ...
      <div className="flex flex-col gap-4 py-4 animate-fade-in">
            ...
            <>
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  <div className="text-[11px] text-white/55">Turn</div>
            ...
            {showCands && (
              <div className="mt-2 flex flex-wrap gap-1.5">
                {candidates.slice(0, 40).map((p) => (
                  <span key={p.id} className="rounded-full border border-white/10 bg-white/[0.03] px-2 py-0.5 text-[11px] text-white/65">
                    {p.name}
                  </span>
                ))}
                {candidates.length > 40 && <span className="text-[11px] text-white/55">+{candidates.length - 40} more</span>}
              </div>
            )}
          </Card>
        )}
        ...
        {lastEntry && (
          <Card strong className="p-3 animate-rise-in">
            <div className="text-[11px] text-white/55">Latest answer</div>
            <div className="mt-0.5 flex items-baseline justify-between gap-2">
              <span className="min-w-0 truncate text-sm">
                <span className="text-white/55">{playerById(lastEntry.askerId).name}: </span>
        ...
        {/* History */}
        <Card className="p-4">
          <h2 className="mb-2 text-sm font-semibold text-white/65">Question history</h2>
          {s.history.length === 0 ? (
            <p className="text-xs text-white/55">No questions yet — fire away.</p>
          ) : (
            <ol className="flex flex-col gap-1.5">
              {s.history.map((h) => (
                <li key={h.id} className="text-sm">
                  <span className="text-white/55">{playerById(h.askerId).name}: </span>
    ...
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-ink-900/85 px-5 backdrop-blur-sm" onClick={onClose}>
      <div className="w-full max-w-sm rounded-2xl border border-white/10 bg-ink-800 p-5 shadow-elev-2" onClick={(e) => e.stopPropagation()}>
        <div className="mb-2 flex items-center justify-between">
          ...
          <button onClick={onClose} aria-label="Close" className="text-white/55 hover:text-white"><IconClose className="h-5 w-5" /></button>
        </div>
        <p className="mb-2 text-[11px] text-white/55">Your opponent answers this by hand (Yes / No / Unsure). It won’t auto-filter candidates.</p>
        <textarea
          value={text}
  ...
  return createPortal(
    <div className="fixed inset-0 z-[100] flex items-end justify-center bg-ink-900/85 backdrop-blur-sm sm:items-center sm:p-4" onClick={onClose}>
      <div className="max-h-[90vh] w-full max-w-md overflow-y-auto rounded-t-2xl border border-white/10 bg-ink-800 p-5 shadow-elev-2 sm:rounded-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        {chosenP ? (
          <div className="text-center">
            <div className="text-xs text-white/55">Final guess</div>
  ...
  return (
    <div className="flex flex-1 flex-col items-center justify-center gap-5 py-12 text-center animate-fade-in">
      ...
      <Card className="w-full max-w-xs p-4">
        <div className="text-[11px] text-white/55">The mystery player was</div>
