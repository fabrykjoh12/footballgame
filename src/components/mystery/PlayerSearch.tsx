  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="max-h-[46vh] overflow-y-auto pr-1">
        {list.length === 0 ? (
          <p className="py-6 text-center text-sm text-white/55">No players match.</p>
        ) : (
            ...
            {list.map((p) => (
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="truncate text-[10px] text-white/40">{p.clubs.join(', ')}</div>
                </div>
...
function Silhouette({ role }: { role: PlayerRole }) {
  return (
    <span
      aria-hidden
      className="grid h-9 w-9 shrink-0 place-items-center rounded-lg border border-white/10 bg-gradient-to-b from-white/[0.05] to-white/[0.03] text-xs"
    >
