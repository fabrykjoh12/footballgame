...
export function CareerPathGame({ onExit }: { onExit: () => void }) {
  ...
  if (phase === 'over') {
    return (
      <div className="flex flex-1 flex-col items-center justify-center gap-5 py-10 text-center animate-fade-in">
        ...
        <div>
          <div className="text-xs font-bold text-white/55">Career Path</div>
  ...
  if (!player) {
    return <div className="flex flex-1 items-center justify-center py-20 text-white/55">No players available.</div>;
  }
  ...
  return (
    <div className="flex flex-1 flex-col gap-4 py-4 animate-fade-in">
      ...
      <Card strong className="relative overflow-hidden p-4">
        ...
        <div className="mb-2 text-center text-xs text-white/55">Which player is this?</div>
          ...
          {player.clubs.map((club, i) => {
            ...
            return (
              <li key={i} className="flex items-center gap-2">
                <span className="nums w-5 text-right text-xs text-white/40">{i + 1}</span>
                <span
                  className={[
                    'flex flex-1 items-center gap-2 rounded-lg border px-3 py-2 font-display font-bold',
                    shown ? 'border-white/10 bg-white/[0.04] text-white' : 'border-dashed border-white/10 bg-white/[0.03] text-white/40',
                  ].join(' ')}
                >
                  <span
      ...
      {!revealing && (
        ...
        >
          ...
          <button type="button" onClick={giveUp} className="self-center text-[11px] text-white/40 hover:text-white/65">
            Give up
          </button>
        </form>
      )}
